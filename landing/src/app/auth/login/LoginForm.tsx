...
export default function LoginForm() {
    ...
    async function handleSSOLogin(provider: "GoogleOAuth" | "MicrosoftOAuth") {
        setIsLoading(true);
        setError("");
        try {
            const res = await fetch("/auth/login/api", {
            ...
            const data = await res.json();
            if (!res.ok) throw new Error(data.error || "Failed to initiate sign in");
            window.location.href = data.authUrl;
        } catch (err) {
            setError(err instanceof Error ? err.message : "Something went wrong");
            setIsLoading(false);
    ...
    return (
        <div className="auth-card">
            ...
            <p className="auth-footer-text">
                ...
                <Link href={`${process.env.NEXT_PUBLIC_LANDING_URL || 'https://edumyles.vercel.app'}/auth/signup`} className="auth-link">Sign up free</Link>
            </p>
