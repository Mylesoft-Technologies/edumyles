...
export default function StudentDashboard() {
    ...
    const gpa = grades ? (grades.reduce((acc, g) => acc + (g.score / g.maxScore) * 100, 0) / (grades.length || 1)).toFixed(1) : "0.0";
    const attendanceRate = attendance ? (attendance.filter(r => r.status === "present").length / (attendance.length || 1) * 100).toFixed(0) : "0";
                        ...
                        <div className="space-y-4">
                            {assignments && assignments.filter(a => a.status === "pending").length > 0 ? (
                                assignments.filter(a => a.status === "pending").slice(0, 3).map((asg) => (
                                    <div key={asg._id} className="flex items-center justify-between border-b pb-3 last:border-0 last:pb-0">
                                        <div>
